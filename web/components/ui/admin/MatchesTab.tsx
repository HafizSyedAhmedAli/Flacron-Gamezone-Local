...
interface MatchesTabProps {
  ...
  onBrowse: () => void;
  // YouTube stream props (optional so existing call sites don't break)
  onFindStream?: (matchId: string) => void;
  findingStreamFor?: string | null;
}
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-4">
        {matches.map((match) => {
          const hasStream =
            match.stream?.isActive && match.stream?.youtubeVideoId;
          const isSearchingThis = findingStreamFor === match.id;
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-2">
                  {/* Stream status pill — shown when a stream is already saved */}
                  {hasStream && (
                    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-xs font-bold bg-red-500/15 border border-red-500/30 text-red-400">
              ...
              {/* Stream title — shown when a stream is saved */}
              {hasStream && match.stream?.streamTitle && (
                <div className="flex items-center gap-1.5 text-xs text-red-400/80 mb-3 truncate">
                  <Youtube className="w-3 h-3 shrink-0" />
                  <span className="truncate">{match.stream.streamTitle}</span>
                </div>
              )}
              ...
              <div className="flex gap-2 flex-wrap">
                ...
                {/* Find Stream — only shown for LIVE matches */}
                {match.status === "LIVE" && onFindStream && (
                  <button
                    onClick={() => onFindStream(match.id)}
                    disabled={isSearchingThis}
                    className="px-4 py-2 bg-gradient-to-r from-red-900/60 to-red-800/60 hover:from-red-700 hover:to-red-600 border border-red-600/40 hover:border-red-500/60 rounded-lg text-sm font-medium transition-all duration-300 hover:scale-105 flex items-center gap-2 text-red-300 hover:text-white disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
                    aria-label={`find-stream-${match.id}`}
                  >
                    {isSearchingThis ? (
