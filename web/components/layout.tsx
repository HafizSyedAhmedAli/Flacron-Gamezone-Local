...
import {
  LogOut, Menu, X, Trophy, Zap, Play, Users, Shield,
  Globe, Mail, Heart, Github, Twitter, Facebook,
  Search, Clock, TrendingUp, ChevronRight, Tag, LayoutDashboard,
} from "lucide-react";
...
// ─── Types ─────────────────────────────────────────────────────────────────────
interface League { id: string; name: string; country: string | null; logo: string; }
interface Team   { id: string; name: string; logo: string | null; }
interface Match  {
  id: string; homeTeam: Team; awayTeam: Team;
  league: League | null; kickoffTime: string;
  status: "UPCOMING" | "LIVE" | "FINISHED";
  score: string | null; venue: string | null;
}
interface SearchResults { leagues: League[]; teams: Team[]; matches: Match[]; }
...
function SearchOverlay({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const [activeFilter,  setActiveFilter]  = useState<"all"|"leagues"|"teams"|"matches">("all");
  ...
  useEffect(() => {
    if (open) {
      ...
      setQuery(""); setResults(null); setActiveFilter("all");
    }
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        setIsSearching(true);
        const res = await apiGet<SearchResults>(`/api/search?q=${encodeURIComponent(query)}`);
        setResults(res);
  ...
  const saveSearch = (term: string) => {
    ...
    try { localStorage.setItem("fgz_recent_searches", JSON.stringify(updated)); } catch {}
  };

  const handleResultClick = (name: string) => { saveSearch(name); onClose(); };

  const totalResults = results
    ? results.leagues.length + results.teams.length + results.matches.length : 0;
  ...
  const FILTERS = [
    { key: "all"     as const, label: "All",     icon: <TrendingUp className="w-3.5 h-3.5" /> },
    { key: "leagues" as const, label: "Leagues", icon: <Trophy     className="w-3.5 h-3.5" /> },
    { key: "teams"   as const, label: "Teams",   icon: <Users      className="w-3.5 h-3.5" /> },
    { key: "matches" as const, label: "Matches", icon: <Play       className="w-3.5 h-3.5" /> },
  ];
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex flex-col">
      ...
      {/* Panel */}
      <div className="relative z-10 w-full max-w-3xl mx-auto mt-16 md:mt-24 px-4 flex flex-col" style={{ maxHeight: "80vh" }}>
        ...
        <div className="relative flex items-center bg-slate-800/90 border-2 border-blue-500/40 rounded-2xl shadow-2xl shadow-blue-500/10 overflow-hidden">
          ...
          <div className="absolute right-3 flex items-center gap-2">
            {isSearching && <div className="w-5 h-5 border-2 border-blue-500/30 border-t-blue-400 rounded-full animate-spin" />}
            {query && !isSearching && (
          ...
          <div className="flex items-center gap-2 mt-4 flex-wrap">
            {FILTERS.map(f => (
              <button key={f.key} onClick={() => setActiveFilter(f.key)}
                ...
                {f.icon}{f.label}
                {f.key !== "all" && (
                  <span className={cn("text-xs rounded-full px-1.5", activeFilter === f.key ? "bg-white/20" : "bg-slate-700")}>
                    {f.key === "leagues" ? results.leagues.length : f.key === "teams" ? results.teams.length : results.matches.length}
                  </span>
                )}
              </button>
            ))}
            <span className="ml-auto text-xs text-slate-500">{totalResults} result{totalResults !== 1 ? "s" : ""}</span>
          </div>
        )}
            ...
            <div className="p-6">
              {recentSearches.length > 0 && (
                <>
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button onClick={() => { setRecentSearches([]); try { localStorage.removeItem("fgz_recent_searches"); } catch {} }}
                  ...
                  <div className="flex flex-wrap gap-2 mb-6">
                    {recentSearches.map((s, i) => (
                      <button key={i} onClick={() => setQuery(s)}
                        ...
                        <Clock className="w-3 h-3 text-slate-500" />{s}
                      </button>
                    ))}
                  </div>
                </>
              ...
              {recentSearches.length === 0 && (
                <div className="text-center py-6 mb-4">
                  ...
                  <p className="text-slate-400 font-medium">Search for leagues, teams or matches</p>
                  <p className="text-slate-600 text-sm mt-1">Type anything to get started</p>
                </div>
              )}
                ...
                <div className="p-4">
                  ...
                  <div className="space-y-1">
                    {results.leagues.map(league => (
                      <Link key={league.id} href={`/leagues/${league.id}`} onClick={() => handleResultClick(league.name)}
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-semibold text-sm text-white group-hover:text-blue-400 transition-colors truncate">{league.name}</p>
                          {league.country && (
                ...
                <div className="p-4">
                  ...
                  <div className="space-y-1">
                    {results.teams.map(team => (
                      <Link key={team.id} href={`/teams/${team.id}`} onClick={() => handleResultClick(team.name)}
                        className="flex items-center gap-4 p-3 rounded-xl hover:bg-slate-800/60 transition-all group">
                        <div className="w-10 h-10 bg-slate-800/60 rounded-xl flex items-center justify-center flex-shrink-0">
                          {team.logo
                            ? <img src={team.logo} alt={team.name} className="w-8 h-8 object-contain" />
                            : <span className="text-xs font-black text-slate-400">{team.name.substring(0, 2).toUpperCase()}</span>
                          }
                        </div>
                        <p className="font-semibold text-sm text-white group-hover:text-cyan-400 transition-colors flex-1 truncate">{team.name}</p>
                ...
                <div className="p-4">
                  ...
                  <div className="space-y-1">
                    {results.matches.map(match => (
                      <Link key={match.id} href={`/match/${match.id}`} onClick={() => handleResultClick(`${match.homeTeam.name} vs ${match.awayTeam.name}`)}
                        className="flex items-center gap-4 p-3 rounded-xl hover:bg-slate-800/60 transition-all group">
                        ...
                        <div className="flex items-center gap-2 flex-shrink-0">
                          ...
                          {match.score && (
                            <span className="text-sm font-bold text-white bg-slate-700/60 px-2 py-0.5 rounded-lg">{match.score}</span>
                          )}
                        </div>
        ...
        <p className="text-center text-slate-700 text-xs mt-3 pb-2">
          Press <kbd className="px-1.5 py-0.5 bg-slate-800 rounded text-slate-500 font-mono">ESC</kbd> to close
        </p>
      </div>
    </div>
  );
...
export function Shell({ children, className }: ShellProps) {
  ...
  useEffect(() => {
    const refreshAuth = () => { setIsAuthenticated(checkAuth()); setUserIsAdmin(isAdmin()); };
    refreshAuth();
    const onFocus      = () => refreshAuth();
    const onVisibility = () => { if (document.visibilityState === "visible") refreshAuth(); };
    const onStorage    = (e: StorageEvent) => { if (!e.key || e.key === "fgz_user" || e.key === "fgz_token") refreshAuth(); };
  ...
  const handleLogout = () => { authLogout(); setIsAuthenticated(false); setUserIsAdmin(false); };
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <Link href="/" className="flex items-center gap-3 group">
              ...
              <div className="hidden sm:block">
                <div className="font-display font-black text-xl leading-none">
                  <span className="bg-gradient-to-r from-white to-blue-200 bg-clip-text text-transparent">Flacron</span>
                  <br />
            ...
            <nav className="hidden md:flex items-center gap-1">
              <NavLink href="/"        active={pathname === "/"}        icon={<Zap             className="w-4 h-4" />}>Home</NavLink>
              <NavLink href="/live"    active={pathname === "/live"}    icon={<Play            className="w-4 h-4" />}>Live</NavLink>
              <NavLink href="/matches" active={pathname === "/matches"} icon={<Trophy          className="w-4 h-4" />}>Matches</NavLink>
              <NavLink href="/leagues" active={pathname === "/leagues"} icon={<Shield          className="w-4 h-4" />}>Leagues</NavLink>
              <NavLink href="/teams"   active={pathname === "/teams"}   icon={<Users           className="w-4 h-4" />}>Teams</NavLink>
              <NavLink href="/pricing" active={pathname === "/pricing"} icon={<Tag             className="w-4 h-4" />}>Pricing</NavLink>
              {isAuthenticated && (
                <NavLink
            ...
            <div className="flex items-center gap-2">
              ...
              <Link href="/contact" aria-label="Contact us" className="hidden sm:inline-flex">
                <Button variant="ghost" size="sm" className="hover:bg-slate-800/50 p-2 rounded-md">
              ...
              {isAuthenticated ? (
                ...
                <>
                  <Link href="/login">
                    <Button variant="ghost" size="sm" className="hidden sm:inline-flex hover:bg-blue-500/10 hover:text-blue-400 transition-colors">
                      Login
                    </Button>
                  </Link>
                  <Link href="/signup">
                    <Button size="sm" className="hidden sm:inline-flex bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-500 hover:to-cyan-500 text-white shadow-lg shadow-blue-500/20">
                      Sign Up
                    </Button>
                  </Link>
                </>
          ...
          {mobileMenuOpen && (
            <div className="md:hidden mt-4 pb-4 border-t border-slate-700/50 pt-4 animate-in slide-in-from-top-4 duration-300">
              <div className="flex flex-col gap-2">
                <MobileNavLink href="/"        onClick={() => setMobileMenuOpen(false)} icon={<Zap            className="w-4 h-4" />} active={pathname === "/"}>Home</MobileNavLink>
                <MobileNavLink href="/live"    onClick={() => setMobileMenuOpen(false)} icon={<Play           className="w-4 h-4" />} active={pathname === "/live"}>Live</MobileNavLink>
                <MobileNavLink href="/matches" onClick={() => setMobileMenuOpen(false)} icon={<Trophy         className="w-4 h-4" />} active={pathname === "/matches"}>Matches</MobileNavLink>
                <MobileNavLink href="/leagues" onClick={() => setMobileMenuOpen(false)} icon={<Shield         className="w-4 h-4" />} active={pathname === "/leagues"}>Leagues</MobileNavLink>
                <MobileNavLink href="/teams"   onClick={() => setMobileMenuOpen(false)} icon={<Users          className="w-4 h-4" />} active={pathname === "/teams"}>Teams</MobileNavLink>
                <MobileNavLink href="/pricing" onClick={() => setMobileMenuOpen(false)} icon={<Tag            className="w-4 h-4" />} active={pathname === "/pricing"}>Pricing</MobileNavLink>
                {isAuthenticated && (
                  <MobileNavLink
                    href={userIsAdmin ? "/admin" : "/dashboard"}
                ...
                <MobileNavLink href="/contact" onClick={() => setMobileMenuOpen(false)} icon={<Mail           className="w-4 h-4" />} active={pathname === "/contact"}>Contact</MobileNavLink>
        ...
        <div className="relative container mx-auto px-4 py-12">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-8">
            ...
            <div>
              <div className="flex items-center gap-3 mb-4">
                ...
                <div className="font-display font-black leading-none">
                  <span className="text-white">Flacron</span><br />
                  <span className="text-sm bg-gradient-to-r from-blue-400 to-cyan-400 bg-clip-text text-transparent">GameZone</span>
                </div>
              </div>
              <p className="text-sm text-slate-400 mb-4">
                Your ultimate destination for live football matches and comprehensive league coverage.
              </p>
            ...
            <div>
              ...
              <ul className="space-y-2 text-sm">
                ...
                {isAuthenticated && (
                  <FooterLink href={userIsAdmin ? "/admin" : "/dashboard"}>Dashboard</FooterLink>
                )}
              </ul>
            </div>
            ...
            <div>
              ...
              <ul className="space-y-2 text-sm">
                ...
                <li>
                  <a href="mailto:support@flacrongamezone.com"
          ...
          <div className="border-t border-slate-800/50 pt-8 flex flex-col sm:flex-row justify-between items-center gap-4">
            <p className="text-sm text-slate-500">
              &copy; {new Date().getFullYear()} Flacron GameZone. All rights reserved.
            </p>
...
function MobileNavLink({ href, children, onClick, icon, active }: {
  ...
  return (
    <Link href={href} onClick={onClick} className={cn(
      ...
      active ? "text-blue-400 bg-blue-500/10" : "text-slate-300 hover:text-white hover:bg-slate-800/50",
    )}>
      {icon}{children}
    </Link>
  );
...
function FooterLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <li>
      <Link href={href} className="text-slate-400 hover:text-blue-400 transition-colors inline-block">
        {children}
      </Link>
    </li>
  );
